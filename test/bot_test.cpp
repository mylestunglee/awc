...
TEST_F(game_fixture, prepare_ranged_attack_sets_x_y) {
    ...
    prepare_ranged_attack(game, units_const_get_at(&game->units, 2, 3));
...
TEST_F(game_fixture, prepare_direct_attack_maximises_defense) {
    ...
    game->energies[3][2] = 1;
    game->energies[3][4] = 1;
    auto attacker = units_get_at(&game->units, 3, 3);
...
TEST_F(game_fixture, handle_ranged_attack) {
    insert_unit({.health = HEALTH_MAX, .model = MODEL_ARTILLERY, .x = 2, .enabled = true});
    units_select_at(&game->units, 2, 0);
